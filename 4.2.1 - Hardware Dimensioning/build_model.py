...
def build_and_solve_EML(df_mins, df_maxs, user_constraints,
  ...
  # build a list of expressions y_{alg}_memAvg(MB) * b_{alg}
  prod_list = []
  for alg in algs.keys():
      prod_list.append(mdl.get_var_by_name(f'y_{alg}_memAvg(MB)') * 
...
for alg in algs.keys():
    ...
    #df_new = pd.read_csv('{}_trainDatasetNew.csv'.format(alg))
