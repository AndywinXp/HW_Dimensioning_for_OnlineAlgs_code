...
df = pd.read_csv('./CONTINGENCY_trainDataset.csv')

# Removes header entries
df = df[df['nTraces'] != 'nTraces']

# Convert into numbers
df['nTraces'] = df['nTraces'].astype(int)
...
def train_and_test(features, target, df=df, max_depth=None, plot=False, normalization='', error_plot=False, CV=False, save_path=None, algorithm="DecisionTree"):
    ...
    if normalization == 'minmax':
        if len(features) > 1:
            df = df[['nTraces', 'memAvg(MB)', 'time(sec)', 'sol(keuro)', 'PV_mean', 'Load_mean', 'PV_std', 'Load_std']]
        else:
            df = df[['nTraces', 'memAvg(MB)', 'time(sec)', 'sol(keuro)']]
...
features_target_memAvg = { 'features' : ['nTraces'],
                        'target' : 'memAvg(MB)'}
features_target_time = { 'features' : ['nTraces'],
                        'target' : 'time(sec)'}
features_target_cost = { 'features' : ['nTraces'],
                        'target' : 'sol(keuro)'}

features_target_memAvg_inputs = { 'features' : ['nTraces', 'PV_mean', 'PV_std', 'Load_mean', 'Load_std'], 
                        'target' : 'memAvg(MB)'}
features_target_time_inputs = { 'features' : ['nTraces', 'PV_mean', 'PV_std', 'Load_mean', 'Load_std'], 
                        'target' : 'time(sec)'}
features_target_cost_inputs = { 'features' : ['nTraces', 'PV_mean', 'PV_std', 'Load_mean', 'Load_std'], 
                        'target' : 'sol(keuro)'}
